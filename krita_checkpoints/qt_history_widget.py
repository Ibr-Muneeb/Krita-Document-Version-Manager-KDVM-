...
# max number of frames to sample into an animated checkpoint's filmstrip thumbnail
filmstrip_frame_count = 4
    ...
    def __init__(self, utils):
        ...
        self._data = [
            [
                ...
                "{}{}\n{}".format(
                    "\U0001F39E  " if self._history[key].get("animated") else "",
                    self._history[key]["date"],
                    self._history[key]["id"],
                ),
...
class HistoryWidget(QtWidgets.QWidget):
    ...
    def generate_thumbnail_action(self, doc_id):
        ...
        is_animated = self.generate_thumbnail(doc, thumbnail_tgt)
        doc.close()

        # update thumbnail path / animated flag in history.json if needed
        needs_thumbnail_update = self.model.history[doc_id]["thumbnail"] != "thumbnail.png"
        needs_animated_update = self.model.history[doc_id].get("animated") != is_animated
        if needs_thumbnail_update or needs_animated_update:
            ...
            vmutils.history[doc_id]["animated"] = is_animated
    ...
    def add_checkpoint(self, msg="", autosave=False, generate_thumbnail=True):
        ...
        if generate_thumbnail:
            ...
            is_animated = self.generate_thumbnail(doc, filename)
    ...
    def generate_thumbnail(self, doc, filename):
        """Opens a krita document and generates a new thumbnail image.

        Animated documents (animationLength() > 1) get a filmstrip-style
        thumbnail: a handful of evenly-spaced frames composited side by
        side, so animated checkpoints are recognizable at a glance in the
        history panel without needing an external GIF dependency.
        ...
        Returns:
            bool - True if the document has animation content.
        """

        self.in_progress.emit(True)

        anim_length = doc.animationLength() if hasattr(doc, "animationLength") else 0
        is_animated = bool(anim_length) and anim_length > 1

        if is_animated:
            frame_count = min(filmstrip_frame_count, anim_length)
            if frame_count > 1:
                frame_times = [
                    int(round(i * (anim_length - 1) / (frame_count - 1)))
                    for i in range(frame_count)
                ]
            else:
                frame_times = [0]

            self.status_update(f"Sampling {len(frame_times)} animation frames")
            frames = [self._render_frame(doc, t) for t in frame_times]
            self.status_update(f"saving filmstrip thumbnail to {filename}")
            self._save_filmstrip(frames, filename)
        else:
            self._save_single_frame(doc, filename)
            self.status_update(f"saving thumbnail to {filename}")

        self.in_progress.emit(False)
        return is_animated

    def _render_frame(self, doc, time):
        """Renders a single animation frame of doc to a scaled QPixmap.

        Parameters:
            doc (Krita static instance) - source document
            time (int) - animation frame index to render
        ...
        clone.setCurrentTime(time)
        clone.refreshProjection()
        clone.flatten()
        self._scale_to_thumbnail(clone)

        tmp_path = os.path.join(
            tempfile.gettempdir(), f"checkpoint_frame_{time}_{id(clone)}.png"
        )
        clone.exportImage(tmp_path, krita.InfoObject())
        clone.close()
    ...
    def _save_filmstrip(self, frame_pixmaps, filename):
        """Composites a list of QPixmap frames side by side into one PNG.

        Parameters:
            frame_pixmaps (list[QtGui.QPixmap])
            filename (str) - path to save the composited PNG to
        """

        if not frame_pixmaps:
            return

        gap = 4
        height = max(pixmap.height() for pixmap in frame_pixmaps)
        total_width = sum(pixmap.width() for pixmap in frame_pixmaps) + gap * (
            len(frame_pixmaps) - 1
        )

        filmstrip = QtGui.QPixmap(total_width, height)
        filmstrip.fill(QtCore.Qt.transparent)

        painter = QtGui.QPainter(filmstrip)
        x_offset = 0
        for pixmap in frame_pixmaps:
            painter.drawPixmap(x_offset, (height - pixmap.height()) // 2, pixmap)
            x_offset += pixmap.width() + gap
        painter.end()

        filmstrip.save(filename, "PNG")

    def _save_single_frame(self, doc, filename):
        """Flattens doc and exports a single static thumbnail image.
    ...
    def _scale_to_thumbnail(self, clone):
        """Scales an open document clone so its longest side matches
        default_thumbnail_resolution, in place.

        Parameters:
            clone (Krita static instance) - document clone to scale
        """
