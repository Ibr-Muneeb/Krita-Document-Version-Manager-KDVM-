...
class Utils(QtCore.QObject):
    ...
    def add_checkpoint(self, msg=""):
        ...
        if os.path.exists(doc_dir):
            self.unlock_history()
            raise FileExistsError(
                "No modifications to save. A checkpoint for this "
                f"timestamp already exists. {date_string}"
            )
        ...
        if os.name == "nt":
            # TODO: resolve the real file owner on Windows via win32security.
            # Left blank for now rather than shipping unused imports.
            owner = ""
        else:
